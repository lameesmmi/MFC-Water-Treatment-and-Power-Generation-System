...
interface PostTreatmentPanelProps {
  voltage: number;
  current: number;
  connected: Record<string, boolean>;
}

export function PostTreatmentPanel({ voltage, current, connected }: PostTreatmentPanelProps) {
  const voltageStatus = !connected.voltage ? 'offline' : voltage > 0 && voltage <= 50 ? 'safe' : voltage > 50 ? 'high' : 'low';
  const currentStatus = !connected.current ? 'offline' : current > 0 && current <= 5  ? 'safe' : current > 5  ? 'high' : 'low';
